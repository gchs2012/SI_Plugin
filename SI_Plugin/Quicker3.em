...
macro ExpandProcEN(szMyName,wordinfo,szLine,szLine1,nVer,ln,sel)
{
    ...
    {
        SetBufSelText(hbuf, " (/* condition */)")
        InsBufLine(hbuf, ln + 1, "@szLine1@" # "{");
        InsBufLine(hbuf, ln + 2, "@szLine@" # "/* code */");
        InsBufLine(hbuf, ln + 3, "@szLine1@" # "}");
...
macro ExpandProcCN(szMyName,wordinfo,szLine,szLine1,nVer,ln,sel)
{
    ...
    {
        SetBufSelText(hbuf, " (i = 0; i < count; i++)")
        InsBufLine(hbuf, ln + 1, "@szLine1@" # "{")
        InsBufLine(hbuf, ln + 2, "@szLine@" # "#;")
        InsBufLine(hbuf, ln + 3, "@szLine1@" # "}")
        SetWndSel(hwnd, sel)
        ...
        newsel.ichLim = GetBufLineLength (hbuf, ln)
        SetWndSel(hwnd, newsel)
        SetBufSelText(hbuf, " (@szVar@ = 0; @szVar@ < count; @szVar@++)")
    }
    else if (szCmd == "fo")
    {
        SetBufSelText(hbuf, "r (i = 0; i < count; i++)")
        InsBufLine(hbuf, ln + 1, "@szLine1@" # "{")
        InsBufLine(hbuf, ln + 2, "@szLine@" # "#;")
        InsBufLine(hbuf, ln + 3, "@szLine1@" # "}")
        symname =GetCurSymbol ()
...
macro InsertFileHeaderCN(hbuf, ln, szName, szContent)
{
    ...
    InsBufLine(hbuf, ln + 0,  "/******************************************************************************")
    InsBufLine(hbuf, ln + 1,  "")
    InsBufLine(hbuf, ln + 2,  "            版权所有 (C), 2015-2016, 西安交大捷普网络科技有限公司")
    InsBufLine(hbuf, ln + 3,  "")
    InsBufLine(hbuf, ln + 4,  " ******************************************************************************")
    sz = GetFileName(GetBufName (hbuf))
    ...
    //插入函数列表
    ln = InsertFileList(hbuf,hnewbuf,ln + 10) - 10
    closebuf(hnewbuf)
...
macro FuncHeadCommentCN(hbuf, ln, szFunc, szMyName,newFunc)
{
    ...
    {
        InsBufLine(hbuf, ln+8, "UINT @szFunc@( # )")
    ...
    if ((newFunc == 1) && (strlen(szFunc)>0))
    {
        ...
        while (1)
        {
            szParam = ask("请输入参数类型和参数名: INT iNum")
...
//            nLeft = GetLeftBlank(szLine)
            ...
            {
                ln = ln + 4
                InsBufLine(hbuf, ln - 2, "@szLeft@	" # "case #:")
                InsBufLine(hbuf, ln - 1, "@szLeft@	" # "	" # "#;")
...
macro InsertReviseAdd()
{
    ...
    if(strlen(szQuestion)>0)
    {
        InsBufLine(hbuf, sel.lnFirst, "@szLeft@/* BEGIN: Added by @szMyName@, @sz@/@sz1@/@sz3@   问题单号: @szQuestion@ */");
    }
    else
    {
    ...
    if(strlen(szQuestion)>0)
    {
        InsBufLine(hbuf, sel.lnLast + 2, "@szLeft@/* END:   Added by @szMyName@, @sz@/@sz1@/@sz3@   问题单号: @szQuestion@ */");            
    }
    else
    {
...
macro InsertReviseDel()
{
    ...
    if(strlen(szQuestion)>0)
    {
        InsBufLine(hbuf, sel.lnFirst, "@szLeft@/* BEGIN: Deleted by @szMyName@, @sz@/@sz1@/@sz3@   问题单号: @szQuestion@ */");
    }
    else
    {
    ...
    if(strlen(szQuestion)>0)
    {
        InsBufLine(hbuf, sel.lnLast + 2, "@szLeft@/* END:   Deleted by @szMyName@, @sz@/@sz1@/@sz3@   问题单号: @szQuestion@ */");            
    }
    else
    {
...
macro InsertReviseMod()
{
    ...
    if(strlen(szQuestion)>0)
    {
        InsBufLine(hbuf, sel.lnFirst, "@szLeft@/* BEGIN: Modified by @szMyName@, @sz@/@sz1@/@sz3@   问题单号: @szQuestion@ */");
    }
    else
    {
    ...
    if(strlen(szQuestion)>0)
    {
        InsBufLine(hbuf, sel.lnLast + 2, "@szLeft@/* END:   Modified by @szMyName@, @sz@/@sz1@/@sz3@   问题单号: @szQuestion@ */");            
    }
    else
    {
